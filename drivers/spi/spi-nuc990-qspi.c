...
static int nuvoton_spi_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32_array(pdev->dev.of_node, "reg", val32, 2) != 0) {
		dev_err(&pdev->dev, "can not get bank!\n");
		return -EINVAL;
	}

	nuvoton->phyaddr = val32[0];
...
static const struct of_device_id nuvoton_qspi_of_match[] = {
	{ .compatible = "nuvoton,nuc990-qspi", },
	{ /* sentinel */ }
};
...
MODULE_DESCRIPTION("Nuvoton NUC990 QSPI Controller driver!");
