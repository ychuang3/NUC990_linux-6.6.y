...
static struct nu_sha_dev *nuc990_sha_find_dev(struct nu_sha_ctx *tctx)
{
...
static inline void nuc990_write_reg(struct nu_sha_dev *sha_dd, u32 val, u32 reg)
{
	writel_relaxed(val, sha_dd->reg_base + reg);
}

static inline u32 nuc990_read_reg(struct nu_sha_dev *sha_dd, u32 reg)
{
	return readl_relaxed(sha_dd->reg_base + reg);
}

static int nuc990_sha_dma_run(struct nu_sha_dev *dd, int is_key_block)
{
	...
	if (is_key_block) {
		...
		dma_sync_single_for_device(dd->dev, ctx->dma_buff,
					   HMAC_KEY_BUFF_SIZE, DMA_TO_DEVICE);
		...
		dma_sync_single_for_device(dd->dev, ctx->dma_buff, SHA_BUFF_SIZE,
					   DMA_TO_DEVICE);
		dma_cnt = ctx->bufcnt;
	}
	...
	if ((tctx->hash_mode & HMAC_CTL_SHA3EN) && (ctx->bufcnt == 0)) {
		/* workaround for nuc990 SHA3 in case of DMACNT is 0 */
		ctx->reg_ctl |= HMAC_CTL_DMACSCAD;
	}
	...
	nuc990_write_reg(dd, 0, HMAC_KSCTL);

	nuc990_write_reg(dd, (INTSTS_HMACIF | INTSTS_HMACEIF), INTSTS);
	nuc990_write_reg(dd, nuc990_read_reg(dd, INTEN) | (INTEN_HMACIEN |
			 INTEN_HMACEIEN), INTEN);

	nuc990_write_reg(dd, tctx->hmac_key_len, HMAC_KEYCNT);
	nuc990_write_reg(dd, dma_cnt, HMAC_DMACNT);
	nuc990_write_reg(dd, ctx->dma_buff, HMAC_SADDR);
	nuc990_write_reg(dd, ctx->dma_fdbck, HMAC_FBADDR);
	nuc990_write_reg(dd, ctx->reg_ctl, HMAC_CTL);
 ...
 */
static void  nuc990_sha_get_result(struct ahash_request *req)
{
	...
	for (i = 0; i < ctx->digest_len/4; i++)
		result[i] = nuc990_read_reg(ctx->dd, HMAC_DGST(i));
 ...
 */
static void nuc990_sha_finish_req(struct nu_sha_reqctx *ctx, int err)
{
...
static int nuc990_sha_init(struct ahash_request *req)
{
	struct crypto_ahash *tfm = crypto_ahash_reqtfm(req);
	struct nu_sha_ctx *tctx = crypto_ahash_ctx(tfm);
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
	struct nu_sha_dev *dd = nuc990_sha_find_dev(tctx);
...
static void nuc990_sha_sg_to_dma_buffer(struct ahash_request *req,
					struct nu_sha_reqctx *ctx)
{
	...
	while (ctx->sg && (ctx->req_len > 0) && (ctx->bufcnt < ctx->dma_max_size)) {
		...
		memcpy(&ctx->buffer[ctx->bufcnt], (u8 *)sg_virt(ctx->sg) + ctx->sg_off, copy_len);
...
static int nuc990_sha_update_start(struct nu_sha_dev *dd)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(dd->req);
	int err = 0;

	if ((ctx->req_len > 0) &&  (ctx->bufcnt < ctx->dma_max_size))
		nuc990_sha_sg_to_dma_buffer(dd->req, ctx);

	if (ctx->flags & SHA_FLAGS_KEY_BLK) {
		if ((ctx->flags & (SHA_FLAGS_FINUP | SHA_FLAGS_FINAL)) &&
		    (ctx->bufcnt == 0) && (dd->req->nbytes == 0)) {
			pr_err("nuc990 hmac does not support 0 data length!\n");
			nuc990_sha_finish_req(ctx, -EINVAL);
			return -EINVAL;
		}

		err = nuc990_sha_dma_run(dd, 1);
		if (err != -EINPROGRESS)
			nuc990_sha_finish_req(ctx, err); /* DMA trigger failed, abort! */
		 ...
		 */
		nuc990_sha_finish_req(ctx, 0);
		err = 0;
	}
	return err;
}

static int nuc990_sha_handle_queue(struct nu_sha_dev *dd, struct ahash_request *req)
{
	...
	return nuc990_sha_update_start(dd);
}

static void nuc990_sha_dma_complete(struct nu_sha_reqctx *ctx)
{
	struct nu_sha_dev *dd = ctx->dd;
...
static int nuc990_sha_update(struct ahash_request *req)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
	...
	nuc990_sha_sg_to_dma_buffer(req, ctx);
	if (ctx->bufcnt + ctx->req_len <= ctx->dma_max_size)
		return 0;
	return nuc990_sha_handle_queue(ctx->dd, req);
}

static int nuc990_sha_final(struct ahash_request *req)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
...
static int nuc990_sha_finup(struct ahash_request *req)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
	int err1, err2;
	...
	err1 = nuc990_sha_update(req);
	if (err1 == -EINPROGRESS || (err1 == -EBUSY &&
	    (ahash_request_flags(req) & CRYPTO_TFM_REQ_MAY_BACKLOG)))
		return err1;

	err2 = nuc990_sha_final(req);

	return err1 ?: err2;
}

static int nuc990_sha_digest(struct ahash_request *req)
{
	return nuc990_sha_init(req) ?: nuc990_sha_finup(req);
}

static int nuc990_sha_setkey(struct crypto_ahash *tfm, const u8 *key, u32 keylen)
{
	struct nu_sha_ctx *tctx = crypto_ahash_ctx(tfm);
...
static int nuc990_sha_export(struct ahash_request *req, void *out)
{
...
static int nuc990_sha_import(struct ahash_request *req, const void *in)
{
	struct nu_sha_reqctx *ctx = ahash_request_ctx(req);
...
static int nuc990_sha_cra_init_alg(struct crypto_tfm *tfm, const char *alg_base)
{
	struct nu_sha_ctx *tctx = crypto_tfm_ctx(tfm);
	struct nu_sha_dev *dd = nuc990_sha_find_dev(tctx);

	dd = nuc990_sha_find_dev(tctx);
	if (!dd)
		return -ENODEV;
...
static int nuc990_sha_cra_init(struct crypto_tfm *tfm)
{
	// printk("SHA: %s\n", tfm->__crt_alg->cra_driver_name);

	return nuc990_sha_cra_init_alg(tfm, NULL);
}

static struct ahash_alg nuc990_sha_algs[] = {
{
	.init   = nuc990_sha_init,
	.update = nuc990_sha_update,
	.final  = nuc990_sha_final,
	.finup  = nuc990_sha_finup,
	.digest = nuc990_sha_digest,
	.export = nuc990_sha_export,
	.import = nuc990_sha_import,
	.halg.digestsize = SHA1_DIGEST_SIZE,
	.halg.statesize = sizeof(struct nu_sha_reqctx),
	.halg.base = {
		.cra_name        = "sha1",
		.cra_driver_name = "nuc990-sha1",
		.cra_priority    = 400,
...
},
{
	...
	.export = nuc990_sha_export,
	.import = nuc990_sha_import,
	.setkey = nuc990_sha_setkey,
	.halg.digestsize = SHA1_DIGEST_SIZE,
	.halg.statesize = sizeof(struct nu_sha_reqctx),
	.halg.base = {
		.cra_name        = "hmac(sha1)",
		.cra_driver_name = "nuc990-hmac-sha1",
		.cra_priority    = 400,
		.cra_flags       = CRYPTO_ALG_ASYNC,
		.cra_blocksize   = SHA1_BLOCK_SIZE,
		.cra_ctxsize     = sizeof(struct nu_sha_ctx),
...
},
{
	...
	.halg.base = {
		...
		.cra_driver_name = "nuc990-hmac-sha224",
		.cra_priority    = 400,
		.cra_flags       = CRYPTO_ALG_ASYNC,
		.cra_blocksize   = SHA224_BLOCK_SIZE,
		.cra_ctxsize     = sizeof(struct nu_sha_ctx),
...
},
{
	...
	.halg.base = {
		...
		.cra_driver_name = "nuc990-hmac-sha256",
		.cra_priority    = 400,
		.cra_flags       = CRYPTO_ALG_ASYNC,
		.cra_blocksize   = SHA256_BLOCK_SIZE,
		.cra_ctxsize     = sizeof(struct nu_sha_ctx),
...
},
{
	...
	.halg.base = {
		...
		.cra_driver_name = "nuc990-hmac-sha384",
		.cra_priority    = 400,
		.cra_flags       = CRYPTO_ALG_ASYNC,
		.cra_blocksize   = SHA384_BLOCK_SIZE,
		.cra_ctxsize     = sizeof(struct nu_sha_ctx),
...
},
{
	...
	.halg.base = {
		...
		.cra_driver_name = "nuc990-hmac-sha512",
		.cra_priority    = 400,
		.cra_flags       = CRYPTO_ALG_ASYNC,
		.cra_blocksize   = SHA512_BLOCK_SIZE,
		.cra_ctxsize     = sizeof(struct nu_sha_ctx),
...
static struct ahash_alg  nuc990_sha3_algs[] = {
{
	.init   = nuc990_sha_init,
	.update = nuc990_sha_update,
	.final  = nuc990_sha_final,
	...
	.halg.base = {
		.cra_name        = "sha3-224",
		.cra_driver_name = "nuc990-sha3-224",
		.cra_priority    = 400,
		.cra_flags       = CRYPTO_ALG_ASYNC,
...
},
{
	...
	.halg.base = {
		.cra_name        = "sha3-384",
		.cra_driver_name = "nuc990-sha3-384",
		.cra_priority    = 400,
		.cra_flags       = CRYPTO_ALG_ASYNC,
...
static void nuc990_sha_queue_task(unsigned long data)
{
	struct nu_sha_dev *dd = (struct nu_sha_dev *)data;

	nuc990_sha_handle_queue(dd, NULL);
}

/*
 *  This task is triggerred by Crypto IRQ when a SHA DMA completed.
 */
static void nuc990_sha_done_task(unsigned long data)
{
	struct nu_sha_dev *dd = (struct nu_sha_dev *)data;
	struct nu_sha_reqctx *ctx = ahash_request_ctx(dd->req);
	int map_size;
	...
	dma_sync_single_for_cpu(dd->dev, ctx->dma_fdbck, SHA_FDBCK_SIZE, DMA_FROM_DEVICE);
...
int nuc990_sha_probe(struct device *dev, struct nu_crypto_dev *crypto_dev)
{
	...
	tasklet_init(&sha_dd->done_task, nuc990_sha_done_task, (unsigned long)sha_dd);
	tasklet_init(&sha_dd->queue_task, nuc990_sha_queue_task, (unsigned long)sha_dd);
	...
	for (i = 0; i < ARRAY_SIZE(nuc990_sha_algs); i++) {
		err = crypto_register_ahash(&nuc990_sha_algs[i]);
		if (err)
			goto err_register;
	}

	for (i = 0; i < ARRAY_SIZE(nuc990_sha3_algs); i++) {
		err = crypto_register_ahash(&nuc990_sha3_algs[i]);
		if (err)
			goto err_register;
	}

	pr_info("nuc990 crypto sha engine initialized.\n");
	return 0;
...
err_register:
	...
	for (i = 0; i < ARRAY_SIZE(nuc990_sha_algs); i++)
		crypto_unregister_ahash(&nuc990_sha_algs[i]);

	for (i = 0; i < ARRAY_SIZE(nuc990_sha3_algs); i++)
		crypto_unregister_ahash(&nuc990_sha3_algs[i]);
