...
static struct nu_ecc_dev *nuc990_ecc_find_dev(struct nu_ecc_ctx *tctx)
{
...
static inline void nuc990_write_reg(struct nu_ecc_dev *ecc_dd, u32 val, u32 reg)
{
	writel_relaxed(val, ecc_dd->reg_base + reg);
}

static inline u32 nuc990_read_reg(struct nu_ecc_dev *ecc_dd, u32 reg)
{
	return readl_relaxed(ecc_dd->reg_base + reg);
}

static inline void nuc990_write_ecc_key(struct nu_ecc_ctx *ctx, u8 *key, u32 reg)
{
	...
	for (ri = 0; (ri < 18) && (i >= 0); ri++) {
		...
		val32 |= key[i];
		i -= 4;
		nuc990_write_reg(dd, val32, reg + ri * 4);
	}
}

static inline void nuc990_read_ecc_key(struct nu_ecc_ctx *ctx, u32 reg, u8 *key)
{
	struct nu_ecc_dev *dd = ctx->dd;
	u32 val32;
	int i, wcnt, idx;
	...
	val32 = nuc990_read_reg(dd, reg + idx * 4);
	i = 0;
	if (ctx->keylen % 4 == 1) {
		key[i++] = val32 & 0xff;
		idx--;
...
static inline int nuc990_wait_ecc_complete(struct nu_ecc_dev *dd, int timeout)
{
	unsigned long t_out;

	t_out = jiffies + timeout;
	while (nuc990_read_reg(dd, ECC_STS) & (ECC_STS_BUSY | ECC_STS_DMABUSY)) {
		if (!time_before(jiffies, t_out)) {
			nuc990_write_reg(dd, ECC_CTL_STOP, ECC_CTL);
			pr_err("ECC hardware timeout! 0x%x\n", nuc990_read_reg(dd, ECC_STS));
			return -EBUSY;
		}

		if (nuc990_read_reg(dd, ECC_STS) &
...
static int nuc990_ecc_init_curve(int curve_id, struct nu_ecc_ctx *ctx)
{
	struct nu_ecc_dev *dd = ctx->dd;
	...
	for (i = 0; i < 18; i++) {
		nuc990_write_reg(dd, 0, ECC_X1 + i * 4);
		nuc990_write_reg(dd, 0, ECC_Y1 + i * 4);
		nuc990_write_reg(dd, 0, ECC_X2 + i * 4);
		nuc990_write_reg(dd, 0, ECC_Y2 + i * 4);
		nuc990_write_reg(dd, 0, ECC_A + i * 4);
		nuc990_write_reg(dd, 0, ECC_B + i * 4);
		nuc990_write_reg(dd, 0, ECC_N + i * 4);
		nuc990_write_reg(dd, 0, ECC_K + i * 4);
	}

	nuc990_write_ecc_key(ctx, curve->Px, ECC_X1);
	nuc990_write_ecc_key(ctx, curve->Py, ECC_Y1);
	nuc990_write_ecc_key(ctx, curve->Ea, ECC_A);
	nuc990_write_ecc_key(ctx, curve->Eb, ECC_B);

	if (curve->GF == CURVE_GF_2M) {
		nuc990_write_reg(dd, 0x1, ECC_N);

		reg = ECC_N + (curve->key_len / 32) * 4;
		nuc990_write_reg(dd, nuc990_read_reg(dd, reg) |
		...
		nuc990_write_ecc_key(ctx, curve->Pp, ECC_N);
	}
	return 0;
}

static int nuc990_ecc_point_mult(struct nu_ecc_ctx *ctx, u8 *private_key,
			       u8 *public_key, u8 *result)
{
	...
	if (public_key != NULL) {
		nuc990_write_ecc_key(ctx, public_key, ECC_X1);
		nuc990_write_ecc_key(ctx, public_key + ctx->keylen, ECC_Y1);

	} else {
		/* use G point */
	}

	nuc990_write_reg(dd, 0, ECC_KSCTL);
	nuc990_write_reg(dd, 0, ECC_KSXY);

	nuc990_write_ecc_key(ctx, private_key, ECC_K);

	nuc990_write_reg(dd, ((ctx->keylen * 8) << ECC_CTL_CURVEM_OFFSET) |
			 ECC_CTL_FSEL | ECCOP_POINT_MUL | ECC_CTL_START, ECC_CTL);

	err = nuc990_wait_ecc_complete(dd, 2000);
	if (err)
		return err;

	nuc990_read_ecc_key(ctx, ECC_X1, result);
	nuc990_read_ecc_key(ctx, ECC_Y1, result + ctx->keylen);
	return 0;
}

static int nuc990_ecdh_set_secret(struct crypto_kpp *tfm, const void *buf, unsigned int len)
{
	struct nu_ecc_ctx *ctx = kpp_tfm_ctx(tfm);
	struct nu_ecc_dev *dd = nuc990_ecc_find_dev(ctx);
	struct ecdh params;
	int err;
	...
	err = nuc990_ecc_init_curve(ctx->curve_id, ctx);
	if (err)
		return err;

	if (!params.key || !params.key_size)
		return ecc_gen_privkey(ctx->curve_id, ctx->ndigits, (u64 *)ctx->private_key);
...
static int nuc990_ecdh_compute_value(struct kpp_request *req)
{
	...
	ret = nuc990_ecc_init_curve(ctx->curve_id, ctx);
	if (ret)
		return ret;
	...
	if (req->src) {
		...
		ret = nuc990_ecc_point_mult(ctx, ctx->private_key, public_key, shared_secret);

		buf = shared_secret;
	} else {
		/* Make public key by private key and G */
		ret = nuc990_ecc_point_mult(ctx, ctx->private_key, NULL, public_key);
		buf = public_key;
...
static unsigned int nuc990_ecdh_max_size(struct crypto_kpp *tfm)
{
	struct nu_ecc_ctx *ctx = kpp_tfm_ctx(tfm);
...
static int nuc990_ecdh_p192_init_tfm(struct crypto_kpp *tfm)
{
	struct nu_ecc_ctx *ctx = kpp_tfm_ctx(tfm);
...
static struct kpp_alg nuc990_ecdh_p192 = {
	.set_secret = nuc990_ecdh_set_secret,
	.generate_public_key = nuc990_ecdh_compute_value,
	.compute_shared_secret = nuc990_ecdh_compute_value,
	.max_size = nuc990_ecdh_max_size,
	.init = nuc990_ecdh_p192_init_tfm,
	.base = {
		.cra_flags = CRYPTO_ALG_NEED_FALLBACK,
		.cra_name = "ecdh-nuc990-p192",
		.cra_driver_name = "ecdh-p192-nuc990-generic",
		.cra_priority = 400,
...
static int nuc990_ecdh_p256_init_tfm(struct crypto_kpp *tfm)
{
	struct nu_ecc_ctx *ctx = kpp_tfm_ctx(tfm);
...
static struct kpp_alg nuc990_ecdh_p256 = {
	.set_secret = nuc990_ecdh_set_secret,
	.generate_public_key = nuc990_ecdh_compute_value,
	.compute_shared_secret = nuc990_ecdh_compute_value,
	.max_size = nuc990_ecdh_max_size,
	.init = nuc990_ecdh_p256_init_tfm,
	.base = {
		.cra_flags = CRYPTO_ALG_NEED_FALLBACK,
		.cra_name = "ecdh-nuc990-p256",
		.cra_driver_name = "ecdh-p256-nuc990-generic",
		.cra_priority = 400,
		.cra_module = THIS_MODULE,
		.cra_ctxsize = sizeof(struct nu_ecc_ctx),
	},
...
static int nuc990_ecdh_p384_init_tfm(struct crypto_kpp *tfm)
{
	struct nu_ecc_ctx *ctx = kpp_tfm_ctx(tfm);
...
static struct kpp_alg nuc990_ecdh_p384 = {
	.set_secret = nuc990_ecdh_set_secret,
	.generate_public_key = nuc990_ecdh_compute_value,
	.compute_shared_secret = nuc990_ecdh_compute_value,
	.max_size = nuc990_ecdh_max_size,
	.init = nuc990_ecdh_p384_init_tfm,
	.base = {
		.cra_flags = CRYPTO_ALG_NEED_FALLBACK,
		.cra_name = "ecdh-nuc990-p384",
		.cra_driver_name = "ecdh-p384-nuc990-generic",
		.cra_priority = 400,
		.cra_module = THIS_MODULE,
		.cra_ctxsize = sizeof(struct nu_ecc_ctx),
	},
...
static long nuc990_ecc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ECC_IOC_KEY_GEN:
		...
		nuc990_ecc_init_curve(args->curve, ecc_ctx);

		if (args->knum_d != -1) {
			nuc990_write_reg(dd, ECC_KSCTL_RSRCK | (args->knum_d & 0x9F), ECC_KSCTL);
		} else {
			nuc990_write_reg(dd, 0, ECC_KSCTL);
			nuc990_write_ecc_key(ecc_ctx, args->d, ECC_K);
		}
		...
		nuc990_write_reg(dd, (ecc_ctx->curve->key_len << ECC_CTL_CURVEM_OFFSET) |
			       ECCOP_POINT_MUL | ECC_CTL_START | ecc_ctl, ECC_CTL);

		ret = nuc990_wait_ecc_complete(dd, 2000);
		if (ret) {
			kfree(args);
			return ret;
		}

		nuc990_read_ecc_key(ecc_ctx, ECC_X1, kbuf);
		if (copy_to_user(args->out_x, kbuf, ecc_ctx->keylen)) {
			kfree(args);
			return -EFAULT;
		}

		nuc990_read_ecc_key(ecc_ctx, ECC_Y1, kbuf);
		if (copy_to_user(args->out_y, kbuf, ecc_ctx->keylen)) {
			kfree(args);
			return -EFAULT;
		}
		break;
	...
	case ECC_IOC_POINT_MUL:
		...
		nuc990_write_reg(dd, 0, ECC_KSXY);
		nuc990_write_ecc_key(ecc_ctx, args->Qx, ECC_X1);
		nuc990_write_ecc_key(ecc_ctx, args->Qy, ECC_Y1);
...
static int nuc990_ecc_open(struct inode *inode, struct file *file)
{
...
static int nuc990_ecc_close(struct inode *inode, struct file *file)
{
	struct nu_ecc_ctx  *ecc_ctx;
...
static const struct file_operations nuc990_ecc_fops = {
	.owner		= THIS_MODULE,
	.unlocked_ioctl	= nuc990_ecc_ioctl,
	.open           = nuc990_ecc_open,
	.release        = nuc990_ecc_close,
};

static struct miscdevice nuc990_ecc_dev = {
	.minor		= MISC_DYNAMIC_MINOR,
	.name		= "nuvoton-ecc",
	.fops		= &nuc990_ecc_fops,
};
...
int nuc990_ecc_probe(struct device *dev, struct nu_crypto_dev *crypto_dev)
{
	...
	if (crypto_dev->ecc_ioctl == true) {
		misc_register(&nuc990_ecc_dev);
	} else {
		err = crypto_register_kpp(&nuc990_ecdh_p192);
		if (err) {
			dev_err(dev, "register nuc990_ecdh_p192 failed\n");
			goto err_out;
		}

		err = crypto_register_kpp(&nuc990_ecdh_p256);
		if (err) {
			dev_err(dev, "register nuc990_ecdh_p256 failed\n");
			goto err_out;
		}

		err = crypto_register_kpp(&nuc990_ecdh_p384);
		if (err) {
			dev_err(dev, "register nuc990_ecdh_p384 failed\n");
			goto err_out;
		}
	}

	pr_info("nuc990 crypto ecc engine initialized.\n");
	return 0;
...
err_out:
	...
	dev_err(dev, "nuc990 ecc initialization failed. %d\n", err);
	return err;
}
...
int nuc990_ecc_remove(struct device *dev, struct nu_crypto_dev *crypto_dev)
{
	...
	if (crypto_dev->ecc_ioctl == true)
		misc_deregister(&nuc990_ecc_dev);
	else {
		crypto_unregister_kpp(&nuc990_ecdh_p192);
		crypto_unregister_kpp(&nuc990_ecdh_p256);
		crypto_unregister_kpp(&nuc990_ecdh_p384);
	}
