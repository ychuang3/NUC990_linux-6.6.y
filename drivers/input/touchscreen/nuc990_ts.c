...
/*
 * Touchscreen driver for Nuvoton NUC990
 * Touch and normal ADC share the same ADC controller,
 * this driver scheduled two functions to switch between them.
 * 
...
#define ADC_DEFAULT_CLK_RATE	200000
...
static int report_touch(struct nuc990_adc *priv);
static void detect_touch(struct nuc990_adc *priv);
static void detect_pendown(struct nuc990_adc *priv);

static irqreturn_t nuc990_adc_interrupt(int irq, void *private)
{
	struct nuc990_adc *priv = (struct nuc990_adc *)private;
	u32 isr, wkisr;
...
static void detect_penup(struct nuc990_adc *priv)
{
	// Restart a new flow
	priv->pu_handle = 0;
...
static void nuc990_adc_ts_tasklet(struct nuc990_adc *priv)
{
...
{
	struct nuc990_adc *priv = container_of(hrtimer,
						struct nuc990_adc,
						trigger_hrt);
	unsigned long flags;
...
static void detect_wakeup(struct nuc990_adc *priv)
{
	unsigned long flags;
...
static void stop_detect(struct nuc990_adc *priv)
{
	unsigned long flags;
...
static int nuc990_ts_open(struct input_dev *dev)
{
	struct nuc990_adc *priv = input_get_drvdata(dev);
...
static void nuc990_ts_close(struct input_dev *dev)
{
	struct nuc990_adc *priv = input_get_drvdata(dev);
...
static int nuc990_ts_register(struct platform_device *pdev)
{
	struct nuc990_adc *priv = platform_get_drvdata(pdev);
	struct input_dev *ts_dev;
	int ret;
	...
	if (!ts_dev)
		return -ENOMEM;
	ts_dev->name = "nuc990 Touch Screen";
	ts_dev->phys = "nuc990ts/event0";
	ts_dev->id.bustype = BUS_HOST;
	ts_dev->dev.parent = &pdev->dev;
	ts_dev->open = nuc990_ts_open;
	ts_dev->close = nuc990_ts_close;
...
static void nuc990_ts_unregister(struct platform_device *pdev)
{
	struct nuc990_adc *priv = platform_get_drvdata(pdev);
...
static void normal_convert(struct nuc990_adc *priv, int ch)
{
	unsigned long flags;
...
static int nuc990_adc_read_raw(struct iio_dev *idev,
			     struct iio_chan_spec const *chan,
			     int *val, int *val2, long mask)
{
	struct nuc990_adc *priv = iio_device_get_drvdata(idev);
...
static int nuc990_iio_register(struct platform_device *pdev)
{
	int ret, bit, idx = 0;
	struct iio_dev *indio_dev;
	struct nuc990_adc *priv = platform_get_drvdata(pdev);
...
static int nuc990_iio_unregister(struct platform_device *pdev)
{
	struct nuc990_adc *priv = platform_get_drvdata(pdev);
...
static int nuc990_adc_probe_dt(struct nuc990_adc *priv,
			     struct platform_device *pdev)
{
	...
	priv->clk = devm_clk_get(&pdev->dev, "adc0");
	if (IS_ERR(priv->clk))
		return PTR_ERR(priv->clk);

	if (of_property_read_u32(node, "clock-rate", &priv->clk_rate)) {
		dev_warn(&pdev->dev,
			"Missing clock-rate in the DT, set to 200kHz\n");
...
static int nuc990_adc_probe(struct platform_device *pdev)
{
	...
	struct nuc990_adc *priv;
	int ret;

	dev_info(&pdev->dev, "Nuvoton NUC990 ADC Driver\n");
	priv = devm_kzalloc(dev, sizeof(*priv), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;

	ret = nuc990_adc_probe_dt(priv, pdev);
	if (ret) {
	...
	ret = devm_request_irq(&pdev->dev, priv->irq, nuc990_adc_interrupt,
				0, dev_name(dev), priv);
	if (ret) {
	...
	if (priv->enable_ts) {
		ret = nuc990_ts_register(pdev);
		if (ret)
			goto error_disable_clk;
	}

	if (priv->enable_iio) {
		ret = nuc990_iio_register(pdev);
		if (ret)
...
static SIMPLE_DEV_PM_OPS(nuc990_adc_pm_ops,
		nuc990_adc_suspend, nuc990_adc_resume);

static const struct of_device_id nuc990_adc_match[] = {
	{ .compatible = "nuvoton,nuc990-ts" },
    { /* sentinel */ }
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Nuvoton NUC990 Touchscreen ADC Driver");
