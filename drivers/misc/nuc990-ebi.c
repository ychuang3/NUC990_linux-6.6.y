...
#include <uapi/misc/nuc990_ebi.h>
#include "regs-nuc990-ebi.h"
...
static inline void nuc990_set_ebi_ctl(u8 u8Bank)
{
	...
	if (ebi[u8Bank]->csactiveLevel == EBI_CS_ACTIVE_LOW)
		writel_relaxed(readl_relaxed(EBIBaseAddr) &
	...
	if (ebi[u8Bank]->datawidth == EBI_BUSWIDTH_8BIT)
		writel_relaxed(readl_relaxed(EBIBaseAddr) & ~EBI_CTL_DW16_Msk,
			       EBIBaseAddr);
	else
		writel_relaxed(readl_relaxed(EBIBaseAddr) | EBI_CTL_DW16_Msk,
			       EBIBaseAddr);

	TIMIING_CTL = (readl_relaxed(EBIBaseAddr) & ~(EBI_CTL_MCLKDIV_Msk)) |
...
{
	struct nuc990_set_ebi *pebi, sebi;
...
static int ebi_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (remap_pfn_range(vma, vma->vm_start, pageFrameNo, size,
			    vma->vm_page_prot)) {
		pr_err("nuc990_mem_mmap() : remap_pfn_range() failed !\n");
		return -EINVAL;
	}
...
static int nuc990_ebi_probe(struct platform_device *pdev)
{
	unsigned int ch;
	int ret;

	dev_info(&pdev->dev, "NUC990 EBI\n");

	if (of_property_read_u32_array(pdev->dev.of_node, "bank", &ch, 1) !=
	    0) {
	...
	ret = of_property_read_u32_array(pdev->dev.of_node, "csactiveLevel",
					 &ebi[ch]->csactiveLevel, 1);
	if (ret) {
		dev_err(&pdev->dev, "EBI 'csactiveLevel' cannot be read!\n");
		return ret;
	}
	ret = of_property_read_u32_array(pdev->dev.of_node, "datawidth",
					 &ebi[ch]->datawidth, 1);
	if (ret) {
		dev_err(&pdev->dev, "EBI 'datawidth' cannot be read!\n");
		return ret;
	}
	ret = of_property_read_u32_array(pdev->dev.of_node, "tACC",
					 &ebi[ch]->tACC, 1);
	if (ret) {
		dev_err(&pdev->dev,
			"EBI Timing control 'tACC' cannot be read!\n");
		return ret;
	}
	ret = of_property_read_u32_array(pdev->dev.of_node, "tAHD",
					 &ebi[ch]->tAHD, 1);
	if (ret) {
		dev_err(&pdev->dev,
			"EBI Timing control 'tAHD' cannot be read!\n");
		return ret;
	}
	ret = of_property_read_u32_array(pdev->dev.of_node, "IDLE",
					 &ebi[ch]->IDLE, 1);
	if (ret) {
		dev_err(&pdev->dev,
			"EBI Timing control 'IDLE' cannot be read!\n");
		return ret;
	}
	ret = of_property_read_u32_array(pdev->dev.of_node, "MCLKDIV",
					 &ebi[ch]->MCLKDIV, 1);
	if (ret) {
		dev_err(&pdev->dev,
			"EBI Timing control 'MCLKDIV' cannot be read!\n");
		return ret;
	}

	nuc990_set_ebi_ctl(ebi[ch]->bank);
...
static int nuc990_ebi_remove(struct platform_device *pdev)
{
...
static const struct of_device_id nuc990_ebi_of_match[] = {
	{ .compatible = "nuvoton,nuc990-ebi" },
	{},
};
MODULE_DEVICE_TABLE(of, nuc990_ebi_of_match);

static struct platform_driver nuc990_ebi_driver = {
	.driver = {
		.owner  = THIS_MODULE,
		.name = "nuc990-ebi",
		.of_match_table = of_match_ptr(nuc990_ebi_of_match),
	},
	.probe = nuc990_ebi_probe,
	.remove = nuc990_ebi_remove,
};

module_platform_driver(nuc990_ebi_driver);

MODULE_ALIAS("platform:nuc990-ebi");
MODULE_DESCRIPTION("EBI driver for Nuvoton NUC990");
